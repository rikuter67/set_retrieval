...
def parser_run(): # parser for run.py
    ...
    parser.add_argument('-batch_size', type=int, default=20, help='batch size, default=50')
...
if args.model == 'VLAD':
    ...
    model = models.SMN(isCNN=False, is_TrainableMLP=False, is_set_norm=args.is_set_norm, is_cross_norm=args.is_cross_norm, num_layers=args.num_layers, \
...
main_path = os.path.join(main_model_path)
main_checkpoint_path = os.path.join(main_path,"model/cp.ckpt")
main_checkpoint_dir = os.path.dirname(main_checkpoint_path)
main_result_path = os.path.join(main_path,"result/result.pkl")
...
# train_data_set = train_data_set.prefetch(tf.data.AUTOTUNE)
# validation_data_set = validation_data_set.prefetch(tf.data.AUTOTUNE)
#########################################　追加部分　##########################################
...
if not os.path.exists(main_result_path):
    ...
    # history = model.fit(train_generator, epochs=args.epochs, validation_data=((x_valid, x_size_valid), y_valid), shuffle=True, callbacks=[cp_callback,cp_earlystopping])
    history = model.fit(train_generator, epochs=args.epochs, validation_data=((x_valid, x_size_valid), y_valid), shuffle=True, callbacks=[cp_callback,cp_earlystopping])
    ...
    # plot loss & acc
    util.plotLossACC(main_path,loss,val_loss,acc,val_acc)

    # dump to pickle
    with open(main_model_path,'wb') as fp:
        pickle.dump(acc,fp)
        pickle.dump(val_acc,fp)
        pickle.dump(loss,fp)
        pickle.dump(val_loss,fp)
