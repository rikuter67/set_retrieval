...
class SMN(tf.keras.Model):
    def __init__(self, isCNN=True, is_set_norm=False, is_cross_norm=True, is_TrainableMLP=True, num_layers=1, num_heads=2, mode='setRepVec_pivot', calc_set_sim='BERTscore', baseChn=32, baseMlp = 512, rep_vec_num=1, seed_init = 0, cnn_class_num=2, max_channel_ratio=2, is_neg_down_sample=False, use_Cvec=True, is_Cvec_linear=False):
        ...
        #---------------------
        # SetMatchingModelのインスタンス化
        # self.set_matching_model = SetMatchingModel(hidden_units=128)  ###########################################
    ...
    def call(self, x):
        ...
        # XとYのセット間のスコアを計算
        # score = self.set_matching_model(x, y_pred) ###########################################
    ...
    def SetMatching_Loss(real_score, fake_score):
        # 2つのセット間のスコア差に基づく損失を計算
        loss = tf.log(1 + tf.exp(real_score - fake_score))
    ...
    def train_step(self,data):
        ...
        with tf.GradientTape() as tape:
            ...
            predCNN, predSMN, debug = self((x, x_size), training=True) # ここにgalleryを入れるのではだめなのか？
            ...
            # FakeScore - TrueScore = G(X, predSMN)

            # Fakescore = G(X,predSMN)
                # ここが大変かも
            
            # real_score = self.set_matching_model(gallery, x)
            # fake_score = self.set_matching_model(gallery, predSMN)

            # loss = self.SetMatching_Loss(real_score, fake_score)
            # --------------------------------------------
